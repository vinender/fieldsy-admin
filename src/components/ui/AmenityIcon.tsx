...
export const AmenityIcon: React.FC<AmenityIconProps> = ({
    ...
    if (fillColor === '#10301cff') {
        filter = filterBrightGreen;
    } else if (active || fillColor === '#3a6b22') {
        filter = filterDarkGreen;
    }

    // If a custom fillColor is provided that isn't one of our presets, 
    // we can't easily filter it dynamically without a library. 
    // For now, we stick to the requested colors.
